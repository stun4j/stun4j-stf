 ...
 */
@Component
public class HelpStartUpWithMq implements ApplicationContextAware, ApplicationRunner {
  private static final Logger LOG = LoggerFactory.getLogger(HelpStartUpWithMq.class);
    ...
    txnOps.rawExecuteWithoutResult(st -> {
      ...
      try {
        jdbc.update(String.format("insert into stn_stf_sample_mock (id, value) values ('%s', 3)", // 3
            "AppServiceConsumer"));// This defines in which class(simple-name) the error will occur
